...
class GraphBuilder:
    """
    Builds and augments the knowledge graph by adding nodes and edges.
    """
    ...
    def add_fact_edges(self, chunk_ids: List[str], chunk_triples: List[List[Tuple]], ent_node_to_chunk_ids: Dict):
        """
        ...
        Parameters:
            ...
            chunk_triples: List[List[Tuple]]
                A list of lists of tuples representing triples to process. Each triple
                consists of a subject, predicate, and object.
            ent_node_to_chunk_ids: Dict
                Dictionary to update with entity to chunk mappings.
        """
        ...
        for chunk_key, triples in zip(chunk_ids, chunk_triples):
            ...
            if chunk_key not in current_graph_nodes:
                ...
                for node in entities_in_chunk:
                    ent_node_to_chunk_ids[node] = ent_node_to_chunk_ids.get(node, set()).union(set([chunk_key]))
    ...
    def add_passage_edges(self, chunk_ids: List[str], chunk_triple_entities: List[List[str]]) -> int:
        ...
        Returns:
            int: The number of new passage nodes added to the graph.
        """
        from ..utils.misc_utils import compute_mdhash_id
    ...
    def add_synonymy_edges(self, entity_embedding_store: EmbeddingStore, global_config):
        """
        ...
        query_node_key2knn_node_keys = retrieve_knn(query_ids=entity_node_keys,
                                                    ...
                                                    k=global_config.synonymy_edge_topk,
                                                    query_batch_size=global_config.synonymy_edge_query_batch_size,
                                                    key_batch_size=global_config.synonymy_edge_key_batch_size)
        ...
        for node_key in tqdm(query_node_key2knn_node_keys.keys(), total=len(query_node_key2knn_node_keys)):
            ...
            if len(re.sub('[^A-Za-z0-9]', '', entity)) > 2:
                ...
                for nn, score in zip(nns[0], nns[1]):
                    if score < global_config.synonymy_edge_sim_threshold or num_nns > 100:
                        break
